...
let initializeStardewSaveReader = null;

if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  let stardewInitialized = false;
	initializeStardewSaveReader = async function () {
    if (stardewInitialized) return;
    stardewInitialized = true;
    "use strict";
		try {
			const jq = await import('/lib/jquery');
			const $ = jq.default || jq;
			window.$ = $;
		} catch (e) {
			console.warn('Could not load client-side jQuery:', e);
		}

	if (!(window.File && window.FileReader)) {
		const outElement = document.getElementById('out');
		if (outElement) {
			outElement.innerHTML = '<span class="error">Fatal Error: Could not load the File & FileReader APIs</span>';
		}
		return;
	}
	...
	window.__stardewProcessSave = function (file, outputPrefOld = 'hide_details', outputPrefNew = 'hide_all') {
		return new Promise((resolve, reject) => {
			...
			reader.onload = function (e) {
				try {
					...
					const textCandidate = new TextDecoder('utf-8', { fatal: false }).decode(arrayBuffer);

					let xmlString = '';

					if (looksLikeXmlContent(textCandidate)) {
						xmlString = textCandidate;
					} else {
						try {
							const inflated = inflate(arrayBuffer, { to: 'string' });
							if (looksLikeXmlContent(inflated)) {
								xmlString = inflated;
							} else {
								throw new Error('The selected file does not appear to be a Stardew Valley save file.');
							}
						} catch (inflateError) {
							throw new Error(inflateError?.message || 'The selected file could not be read as a Stardew Valley save.');
						}
					}

					if (!xmlString) {
						throw new Error('The selected file does not contain any XML content.');
					}
	...
	function wikimap(item, index, arr) {return wikify(item);}
	...
	function getSectionHeader(saveInfo, title, anchor, showDetailsButton, version) {
		...
		var output = '<div class="save-section" id="wrap_' + anchor + '"><h3>' + title + '</h3><div class="section-content">';
	...
	function parseSummary(xmlDoc, saveInfo) {
		...
		output += '<b>Farm Name</b>: <span class="result">' + saveInfo.farmName + ' Farm</span>';
        output += '<b>Farm Type</b>: <span class="result">' + farmTypes[$(xmlDoc).find('whichFarm').text()] + '</span>';
		output += '<b>Farmer Name</b>: <span class="result">' + farmer + '</span>';
        ...
        output += '<b>Save Version</b>: <span class="result">' + saveInfo.version + ' ' + saveInfo.versionLabel + '</span>';
		output += '<b>Current Day</b>: <span class="result">Day ' + Number($(xmlDoc).find('dayOfMonth').text()) + ' of ' + capitalize($(xmlDoc).find('currentSeason').html()) + ', Year ' + Number($(xmlDoc).find('year').text()) + '</span>';
		output += '<b>Playtime</b>: <span class="result"> ';
	...
	function parseMoney(xmlDoc, saveInfo) {
		var title = 'Money',
			...
			output += '<span class="result">' + $(xmlDoc).find('SaveGame > player > farmName').html() + ' Farm has earned <b>' + addCommas(money) + 'g</b>.</span>\n';
			output += '<ul class="ach_list"><li>';
			output += (money >= 15e3) ? getAchieveString('Greenhorn', 'earn 15,000g', 1) : getAchieveString('Greenhorn', 'earn 15,000g', 0) + addCommas(15e3 - money) + 'g more';
			output += '</li>\n<li>';
			output += (money >= 5e4) ? getAchieveString('Cowpoke', 'earn 50,000g', 1) : getAchieveString('Cowpoke', 'earn 50,000g', 0) + addCommas(5e4 - money) + 'g more';
			output += '</li>\n<li>';
			output += (money >= 25e4) ? getAchieveString('Homesteader', 'earn 250,000g', 1) : getAchieveString('Homesteader', 'earn 250,000g', 0) + addCommas(25e4 - money) + 'g more';
			output += '</li>\n<li>';
			output += (money >= 1e6) ? getAchieveString('Millionaire', 'earn 1,000,000g', 1) : getAchieveString('Millionaire', 'earn 1,000,000g', 0) + addCommas(1e6 - money) + 'g more';
			output += '</li>\n<li>';
			output += (money >= 1e7) ? getAchieveString('Legend', 'earn 10,000,000g', 1) : getAchieveString('Legend', 'earn 10,000,000g', 0) + addCommas(1e7 - money) + 'g more';
			output += '</li></ul></div>';
	...
	function parsePlayerSocial(player, saveInfo, meta) {
		...
		output = '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>SOCIAL PERFECTION</b>: ' + pt_pct + '</span>';
		output += '<span class="result">' + farmer + ' has maxed ' + maxed_count + ' of ' + maxed_total +
				' base game villager relationships.</span>';
	...
	function parsePlayerFamily(player, saveInfo, meta) {
		...
		output += '<span class="result">' + farmer + "'s " + title + ": " + spouse +
			((meta.wedding) ? ' -- wedding in ' + meta.wedding + ' day(s)' : '') + '</span>';
		...
		output += (count >= 3) ? getAchieveString('Full House', 'Married + 2 kids', 1) : getAchieveString('Full House', 'Married + 2 kids', 0) + needs.join(' and ');
		output += '</li></ul></div>';
		table.push(output);
		output = '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + houseType + ' upgraded ' + houseUpgrades + ' time(s) of ';
		output += maxUpgrades + ' possible.</span><ul class="ach_list"><li>';
		output += (houseUpgrades >= 1) ? getAchieveString('Moving Up', '1 upgrade', 1) : getAchieveString('Moving Up', '1 upgrade', 0) + (1 - houseUpgrades) + ' more';
		output += '</li>\n<li>';
		output += (houseUpgrades >= 2) ? getAchieveString('Living Large', '2 upgrades', 1) : getAchieveString('Living Large', '2 upgrades', 0) + (2 - houseUpgrades) + ' more';
		output += '</li>\n<li>';
		output += (houseUpgrades >= maxUpgrades) ? getMilestoneString('House fully upgraded', 1) : getMilestoneString('House fully upgraded', 0) + (maxUpgrades - houseUpgrades) + ' more';
		output += '</li></ul></div>';
		table.push(output);
		return table;
	}
	...
	function parsePlayerCooking(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>COOKING PERFECTION</b>: ' + pt_pct + '</span>';
		output += '<span class="result">' + $(player).children('name').html() + " has cooked " + craft_count + ' and knows ' + known_count + ' of ' + recipe_count + ((mod_known > 0) ? " base game" : "") + ' recipes.</span>\n';
		if (mod_known > 0) {
			output += '<br /><span class="result note">' + $(player).children('name').html() + " has also cooked " +
				mod_craft + ' and knows ' + mod_known + " modded recipes.</span>";
		}
		output += '<ul class="ach_list"><li>';
	...
	function parsePlayerCrafting(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>CRAFTING PERFECTION</b>: ' + pt_pct + '</span>';
		output += '<span class="result">' + $(player).children('name').html() + " has crafted " + craft_count + ' and knows ' +
			known_count + ' of ' + recipe_count + ' base game recipes.</span>';
		if (mod_known > 0) {
	...
	function parsePlayerFishing(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>FISHING PERFECTION</b>: ' + pt_pct + '</span>';
		output += '<span class="result">' + $(player).children('name').html() + ' has ' + count + ' total catches and has caught ' + craft_count + ' of ' + recipe_count + ' base game fish.</span>';
		if (mod_count > 0) {
			output += '<br /><span class="result note">' + $(player).children('name').html() + " has also caught " +
				mod_count + " modded fish.</span>";
		}
		output += '<ul class="ach_list"><li>';
		output += (count >= 100) ? getAchieveString('Mother Catch', 'catch 100 total fish', 1) : getAchieveString('Mother Catch', 'catch 100 total fish', 0) + (100 - count) + ' more';
		output += '</li>\n<li>';
		output += (craft_count >= 10) ? getAchieveString('Fisherman', 'catch 10 different fish', 1) : getAchieveString('Fisherman', 'catch 10 different fish', 0) + (10 - craft_count) + ' more';
		output += '</li>\n<li>';
		output += (craft_count >= 24) ? getAchieveString('Ol\' Mariner', 'catch 24 different fish', 1) : getAchieveString('Ol\' Mariner', 'catch 24 different fish', 0) + (24 - craft_count) + ' more';
		output += '</li>\n<li>';
		if (compareSemVer(saveInfo.version, "1.4") >= 0) {
	...
	function parsePlayerBasicShipping(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>SHIPPING PERFECTION</b>: ' + pt_pct + '</span>';
	...
	function parsePlayerSkills(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>SKILLS PERFECTION</b>:' + pt_pct + '<br /></span>';
		output += 'Farmer Level: <span class="result"><b>' + pt_level + '</b></span>';
        output += 'Farmer Title: <span class="result"><b>' + title + '</b></span>';
		output += '<span class="result">' + saveInfo.players[umid] + ' has reached level 10 in ' + count + ' of 5 skills.</span>';
		output += '<ul class="ach_list"><li>';
		output += (count >= 1) ? getAchieveString('Singular Talent', 'level 10 in a skill', 1) : getAchieveString('Singular Talent', 'level 10 in a skill', 0) + (1 - count) + ' more';
		output += '</li>\n<li>';
		output += (count >= 5) ? getAchieveString('Master of the Five Ways', 'level 10 in every skill', 1) : getAchieveString('Master of the Five Ways', 'level 10 in every skill', 0) + (5 - count) + ' more';
		output += '</li></ul></div>';
	...
	function parsePlayerSkillMastery(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
        output += '<span class="result">' + saveInfo.players[umid] + ' has ' + addCommas(masteryXP) + ' mastery xp.</span>';
		output += '<span class="result">' + saveInfo.players[umid] + ' has maxed ' + maxCount + ' of ' + meta.skills.length + ' skills.</span>';
        output += '<span class="result">' + saveInfo.players[umid] + ' has selected ' + perkCount + ' of ' + meta.skills.length + ' mastery perks.</span>';
		output += '<ul class="ach_list"><li>';
		output += (maxCount >= 5) ? getMilestoneString('Gain access to the Mastery Cave', 1) : getMilestoneString('Gain access to the Mastery Cave', 0) + (meta.skills.length - maxCount) + ' more maxed skills';
        output += '</li></ul>';
		output += '<ul class="ach_list"><li>';
		output += (masteryXP >= 100000) ? getMilestoneString('Reach 100,000 mastery xp', 1) : getMilestoneString('Reach 100,000 mastery xp', 0) + addCommas(masteryNextXP - masteryXP) + ' more xp for next perk unlock and ' + addCommas(100000 - masteryXP) + ' more xp overall';
		output += '</li></ul>';
        output += '<ul class="ach_list"><li>';
		output += (perkCount >= 5) ? getMilestoneString('Acquire all mastery perks', 1) : getMilestoneString('Acquire all mastery perks', 0) + (meta.skills.length - perkCount) + ' more';
	...
	function parseMuseum(xmlDoc, saveInfo) {
		...
		output += '<span class="result">' + intro + ' donated ' + donated_count + ' of ' + museum_count + ' items to the museum.</span><ul class="ach_list">\n';
		output += '<li>';
		output += (donated_count >= 40) ? getAchieveString('Treasure Trove', 'donate 40 items', 1) : getAchieveString('Treasure Trove', 'donate 40 items', 0) + (40 - donated_count) + ' more';
		output += '</li>\n<li>';
		output += (donated_count >= 60) ? getMilestoneString('Donate enough items (60) to get the Rusty Key', 1) : getMilestoneString('Donate enough items (60) to get the Rusty Key', 0) + (60 - donated_count) + ' more';
		output += '</li>\n<li>';
		output += (donated_count >= museum_count) ? getAchieveString('A Complete Collection', 'donate every item', 1) : getAchieveString('A Complete Collection', 'donate every item', 0) + (museum_count - donated_count) + ' more';
		output += '</li></ul></div>';
		if (donated_count < museum_count) {
			meta.hasDetails = true;
			output += '<div class="' + meta.anchor + '_details ' + meta.det_class + '">';
			output += '<span class="need">See below for items left to donate</span><br /></div>';
		}
	...
	function parsePlayerMuseum(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + farmer + ' has found ' + found_art + ' of ' + artifact_count + ' artifacts.</span>';
		output += '<span class="result">' + farmer + ' has found ' + found_min + ' of ' + mineral_count + ' minerals.</span>' + '<ul class="ach_list">\n<li>';
		output += (found_art >= artifact_count) ? getMilestoneString('All artifacts found', 1) : getMilestoneString('All artifacts found', 0) + (artifact_count - found_art) + ' more';
		output += '</li>\n<li>';
		output += (found_min >= mineral_count) ? getMilestoneString('All minerals found', 1) : getMilestoneString('All minerals found', 0) + (mineral_count - found_min) + ' more';
		output += '</li></ul></div>';
	...
	function parsePlayerMonsters(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		if (mineLevel <= 0) {
			output += '<span class="result">' + farmer + ' has not yet explored the mines.</span>';
		} else {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>MONSTER ERADICATION PERFECTION</b>:' + pt_pct + '</span>';
	...
	function parsePlayerQuests(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + $(player).children('name').html() + ' has completed ' + count + ' "Help Wanted" quest(s).</span>';
		output += '<ul class="ach_list"><li>';
	...
	function parsePlayerStardrops(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>STARDROPS PERFECTION</b>: ' + pt_pct + '</span>';
		output += '<span class="result">' + saveInfo.players[umid] + ' has ' + (staminaOverride ? '' : 'not ') + ' reached 508 max stamina (actual amount ' + saveInfo.data[umid].maxStamina + ')</span>';
		if (staminaOverride && count < stardrop_count) {
			output += '<span class="result note">Note: Stamina will override stardrop marker count for achievements and perfection.</span>';
		}
		output += '<span class="result">' + saveInfo.players[umid] + ' has markers for ' + count + ' of ' + stardrop_count + ' stardrops.</span>';
		output += '<ul class="ach_list"><li>';
		output += (count >= stardrop_count) ? getAchieveString('Mystery Of The Stardrops', 'find every stardrop', 1) : getAchieveString('Mystery Of The Stardrops', 'find every stardrop', 0) + (stardrop_count - count) + ' more';
		output += '</li></ul></div>';
		if (need.length > 0) {
			meta.hasDetails = true;
			output += '<div class="' + meta.anchor + '_details ' + meta.det_class + '">';
	...
	function parseGrandpa(xmlDoc, saveInfo) {
		...
		output += '<span class="result">' + farmer + ' has earned a total of ' + addCommas(money) + 'g.</span>';
		output += '<ul class="ach_list"><li>';
		...
		output += '<span class="result">' + farmer + ' has earned ' + Object.keys(ach_have).length +
				' of the ' + ach_count + ' relevant achievements.</span>';
		output += '<ul class="ach_list"><li>';
		...
		output += '<span class="result">' + farmer + ' has ' + realPlayerLevel + ' total skill levels.</span>';
		output += '<ul class="ach_list"><li>';
		...
		output += '<span class="result">' + farmer + ' has ' + heart_count +
				' relationship(s) of 1975+ friendship points (~8 hearts.)</span>';
		output += '<ul class="ach_list"><li>';
		...
		if (hasPet) {
			output += '<span class="result">' + farmer + ' has a pet with ' + petLove + ' friendship points.</span>';
			need = (999 - petLove) + ' more friendship points';
		} else {
			if (petLove > 0) {
				output += '<span class="result">' + farmer + ' previously had a pet with ' + petLove + ' friendship points.</span>'
			} else {
				output += '<span class="result">' + farmer + ' has not had a pet with any friendship points.</span>';
			}
		...
		output += '<span class="result">' + farmer + ((spouse.length > 0) ? ' is' : ' is not') +
				' married and has upgraded the farmhouse ' + houseUpgrades + ' time(s).</span>';
		output += '<ul class="ach_list"><li>';
		need = [];
		if (spouse.length === 0) {
			need.push('a spouse');
		...
		if (hasKeys.length > 0) {
			output += '<span class="result">' + farmer + ' has acquired the ' + hasKeys.join(" and ") + '.</span>';
		} else {
			output += '<span class="result">' + farmer + ' has not acquired either the Rusty Key or Skull Key.</span>';
		}
		output += '<ul class="ach_list"><li>';
	...
	function parseBundles(xmlDoc, saveInfo) {
		...
		if (isJojaMember) {
			if (hybrid) {
				output += '<span class="result">' + farmer + ' completed ' + ccHave + ' Community Center room(s) and then became a Joja member.</span>';
				output += '<span class="result">' + farmer + ' has since completed ' + jojaHave + ' of the remaining ' + hybridLeft + ' projects on the Community Development Form.</span>';
			} else {
				output += '<span class="result">' + farmer + ' is a Joja member and has completed ' + jojaHave + ' of the ' + jojaCount + ' projects on the Community Development Form.</span>';
			}
			hybridLeft -= jojaHave;
			output += '<span class="result">' + farmer + ((hasSeenCeremony) ? ' has' : ' has not') +
					' attended the completion ceremony</span><ul class="ach_list"><li>';
			...
			output += '<span class="result">' + farmer + ' is not a Joja member and has completed ' + ccHave + ' of the ' + ccCount + ' Community Center rooms.</span>';
			output += '<span class="result">' + farmer + ((hasSeenCeremony) ? ' has' : ' has not') + ' attended the completion ceremony</span><ul class="ach_list"><li>';
			if (ccHave === 0) {
	...
	function parsePlayerSecretNotes(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + farmer + ' has ' + (hasSeenKrobus ? '' : 'not ') + ' seen the Shadow Guy at the Bus Stop.</span>';
		output += '<span class="result">' + farmer + ' has ' + (hasMagnifyingGlass ? '' : 'not ') + ' found the Magnifying Glass.</span>';
		...
		output += '<span class="result">' + farmer + ' has read ' + found_notes + ' of ' +
			note_count + ' secret notes.</span>';
		if (mod_count > 0) {
			output += '<span class="result note">' + farmer + ' has read ' + mod_count + ' mod secret note(s).</span>';
		}
		output += '<ul class="ach_list"><li>';
		...
		output += '<span class="result">' + farmer + ' has found the rewards from  ' + found_rewards + ' of ' +
			reward_count + ' secret notes.</span>';
		output += '<ul class="ach_list"><li>';
	...
	function parsePlayerJournalScraps(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + farmer + ' has ' + (hasVisitedIsland ? '' : 'not ') + ' visited the Island.</span>';
		output += '<ul class="ach_list"><li>';
		output += (hasVisitedIsland) ? getAchieveString('A Distant Shore', 'Reach Ginger Island', 1) : getAchieveString('A Distant Shore', 'Reach Ginger Island', 0) + ' to visit'
		output += '</li></ul></div>';
		$(player).find('secretNotesSeen > int').each(function () {
			if (Number($(this).text()) >= 1000) {
		...
		output += '<span class="result">' + farmer + ' has read ' + found_notes + ' of ' + note_count + ' journal scraps.</span>';
		if (mod_count > 0) {
			output += '<span class="result note">' + farmer + ' has read ' + mod_count + ' mod journal scrap(s).</span>';
		}
		output += '<ul class="ach_list"><li>';
		output += (found_notes >= note_count) ? getMilestoneString('Read all the journal scraps', 1) : getMilestoneString('Read all the journal scraps', 0) + (note_count - found_notes) + ' more';
		output += '</li></ul></div>';
		if (found_notes < note_count) {
			for (i = 1; i <= note_count; i++) {
		...
		output = '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + farmer + ' has found the rewards from  ' + found_rewards + ' of ' + reward_count + ' journal scraps.</span>';
		output += '<ul class="ach_list"><li>';
		output += (found_rewards >= reward_count) ? getMilestoneString('Find all the journal scrap rewards', 1) : getMilestoneString('Find all the journal scrap rewards', 0) + (reward_count - found_rewards) + ' more';
		output += '</li></ul></div>';
		if (found_rewards < reward_count) {
			need = [];
	...
	function parseSpecialOrders(xmlDoc, saveInfo) {
		...
		output += '<span class="result">' + intro + ' completed ' + found_count + ' of ' +
			town_count + ' town special orders.</span>';
		output += '<ul class="ach_list"><li>';
	...
	function parseWalnuts(xmlDoc, saveInfo) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">' + '<span class="result"><b>GOLDEN WALNUTS PERFECTION TRACKER</b>: ' + pt_pct + '</span>';
		var intro;
		if (saveInfo.numPlayers > 1) {
			intro = 'Inhabitants of ' + saveInfo.farmName + ' Farm have';
		} else {
		...
		output += '<span class="result">' + intro + ' found ' + game_count + ' of ' + count + ' golden walnuts.</span>';
		if (found_count !== game_count) {
			output += '<span class="result warn">Warning: Save lists a count of ' + game_count + " but we've found markers for " + found_count + '</span>';
		}
		if (game_count < count) {
			output += '<span class="result">The ' + wikify("Golden Parrot", "Golden_Walnut#Golden_Joja_Parrot") + ' will charge ' + addCommas(10000*(count - game_count)) + 'g to collect the rest.</span>';
		} else {
			output += '<span class="result">The ' + wikify("Golden Parrot", "Golden_Walnut#Golden_Joja_Parrot") + (parrotUsed ? ' was' : ' was not') + ' used to finish the collection.</span>';
		}
		output += '<ul class="ach_list"><li>';
	...
	function parsePlayerPowers(player, saveInfo, meta) {
		...
		output += '<span class="result">' + saveInfo.players[umid] + ' has received ' + have_other +
				' of ' + other_count + ' special items &amp; powers.</span>';
		output += '<ul class="ach_list"><li>';
	...
	function parsePlayerArcade(player, saveInfo, meta) {
		...
		output += '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + saveInfo.players[umid] + ' has' + (hasBeatenPK ? '' : ' not') + ' beaten Journey of the Prairie King.</span>';
		var pkProgress = 0;
		...
		if (pkSave.length > 0) {
			...
			output += '<span class="result">' + saveInfo.players[umid] + ' has a JotPK save at stage ' + level + '-' + sublevel + ' (' +
				(($(pkSave).find('died > boolean').text() === 'true') ? 'not ' : '') + 'deathless)</span>';
		...
		output += '<span class="result">' + saveInfo.players[umid] + ' has' + (hasBeatenJK ? '' : ' not') + ' beaten Junimo Kart (Progress Mode).</span>';
		output += '<ul class="ach_list"><li>';
	...
	function parseRaccoons(xmlDoc, saveInfo) {
		...
		output = '<div class="' + meta.anchor + '_summary ' + meta.sum_class + '">';
		output += '<span class="result">' + intro + '</span>';
		...
		output = getSectionHeader(saveInfo, title, anchor, meta.hasDetails, version) + output + getSectionFooter() + '</div>';
		return output;
	}
	...
	function togglePlayer(e) {
		var isOn = ($(e.currentTarget).attr('class') === 'on'), match = "td." + $(e.currentTarget).attr('id').substring(5);
  ...
  window.onload = initializeStardewSaveReader;
  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    initializeStardewSaveReader();
  }
}

function looksLikeXmlContent(text) {
  if (typeof text !== 'string') return false;
  const normalized = text.replace(/^\uFEFF/, '').trimStart();
  return normalized.startsWith('<?xml') || normalized.startsWith('<SaveGame') || normalized.startsWith('<save');
}

export async function processStardewSave(file, outputPrefOld = 'hide_details', outputPrefNew = 'hide_all') {
  	if (typeof window !== 'undefined') {
    	if (typeof initializeStardewSaveReader === 'function') {
      	initializeStardewSaveReader();
    	}

    	if (typeof window.__stardewProcessSave === 'function') {
      	return window.__stardewProcessSave(file, outputPrefOld, outputPrefNew);
    	}
  	} return Promise.reject(new Error('The Stardew save reader is not available in this environment.'));
}

export default function StardewCheckupPage() {
  const [output, setOutput] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [oldPref, setOldPref] = useState('show_all');
  const [newPref, setNewPref] = useState('show_all');
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      const htmlOutput = await processStardewSave(file, oldPref, newPref);
      setOutput(htmlOutput);
    } catch (err) {
      const message =
        err instanceof Error && err.message
          ? err.message
          : 'Failed to parse the save file. Please make sure it is a valid Stardew Valley save.';
    ...
    <main className="tool-page">
      ...
      <section className="tool-hero">
        <div className="wrap">
          ...
          <h1>Save File Reader</h1>
          <p className="lede">A fork of <a href='https://github.com/MouseyPounds/stardew-checkup'><code>MouseyPounds/stardew-checkup</code></a>, with slightly better GUI. Check your progress on achievements, Perfection, relationships, and more.</p>
        </div>
      </section>

      <section className="wrap tool-card">
        <div className="tool-panel">
          Upload your Stardew Valley save file to see detailed information about your game progress. You can find your save files in the following locations:<br />
          <label htmlFor="color-input"><br />Windows: <code>%AppData%/StardewValley/Saves</code></label>
          <label htmlFor="color-input">Mac: <code>~/.config/StardewValley/Saves</code></label>
          <label htmlFor="color-input">Linux: <code>~/.config/StardewValley/Saves</code></label>
          <label htmlFor="color-input">Steam (Proton): <code>~/.steam/steam/steamapps/compatdata/413080/pfx/drive_c/users/$(USER)/AppData/Roaming/StardewValley/Saves</code><br /></label>
          <div
          ...
          >
            ...
            <div style={{ fontSize: '17px', fontWeight: 600, marginBottom: '0.5rem' }}>
              Open or Drag your Save File Here
            </div>
            <div style={{ fontSize: '14px', color: 'var(--muted)' }}>
              Drag and drop or click to select
            </div>
            <div className="mono" style={{ fontSize: '12px', color: 'var(--muted)', marginTop: '1rem' }}>
              Please use the full save file named with your farmer&apos;s name (or farm name) and an ID number (e.g. <code>Shiroharus_388374643</code>); do not use the SaveGameInfo file as it does not contain all the necessary information.
            </div>
            <input
              ref={fileInputRef}
              type="file"
